...
generate_multivariate_normal_random_variable = function(mu,Sigma){
  n = length(mu)
  #1. Generate n standard normal univariate random variables and store in vector z.
  z = rnorm(n,0,1)
  ...
  #   for Cholesky, t(chol(Sigma)) %*% chol(Sigma) = Sigma = (L)(L^T),  to then verify: L %*% t(L)  = Sigma
  L = t(chol(Sigma))
